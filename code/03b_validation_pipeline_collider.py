...
X_cols_no_collider = [col for col in X_cols if col not in ['competitor_sales']]
...
### 6) COMPARe ATE with and without COLLIDER
...
W = df_generated[X_cols_no_collider]
est.fit(Y=Y, T=T, X=None, W=W, cache_values=True)
print(f"calculated price elasticity coeficient without collider: {est.effect(T0=0, T1=1)[0]}")
...
est.fit(Y=Y, T=T, X=None, W=W, cache_values=True)
print(f"calculated price elasticity coeficient with collider: {est.effect(T0=0, T1=1)[0]}")
...
print_dml_coef(df, X_cols=X_cols_no_collider)
print("Elasticity coefficient in original data with collider:")
print_dml_coef(df)


print("Elasticity coefficient in synthetic data data:")
print_dml_coef(train_df, X_cols=X_cols_no_collider)
print("Elasticity coefficient in synthetic data data with collider:")
...
df_test_price_avg_fltrd = df_test_price_avg_fltrd[(df_test_price_avg_fltrd['price'] >= df['price'].min()) & 
                                            (df_test_price_avg_fltrd['price'] <= df['price'].max())]
df_test_price_min_fltrd = df_test_price_min[df_test_price_min['sales'] >= 0]
df_test_price_min_fltrd = df_test_price_min_fltrd[(df_test_price_min_fltrd['price'] >= df['price'].min()) & 
                                            (df_test_price_min_fltrd['price'] <= df['price'].max())]
df_test_price_max_fltrd = df_test_price_max[df_test_price_max['sales'] >= 0]
df_test_price_max_fltrd = df_test_price_max_fltrd[(df_test_price_max_fltrd['price'] >= df['price'].min()) & 
                                            (df_test_price_max_fltrd['price'] <= df['price'].max())]
